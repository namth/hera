...
function initMap(element, input, output, latlng) {
  ...
  map = new google.maps.Map(element, {
    ...
    draggableCursor: 'url("https://hera.ai.vn/wp-content/uploads/2024/04/chuhy-1.png"), crosshair',
  });
  ...
  markers.push(marker);
  putLatLngToInput(map, output); // set default value to input field
  ...
  map.addListener("click", function (e) {
    ...
    map.panTo(marker.position);
    markers.push(marker);
    putLatLngToInput(map, output); // set new position to input field when customer click
  });

  createInfoWindow();

  // Create the search box and link it to the UI element.
  var input = document.getElementById("pac-input").cloneNode(true);
